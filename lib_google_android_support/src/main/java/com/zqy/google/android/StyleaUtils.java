...
//    }
    ...
    public static int getStyleableId(String styleableName) {
        Context mContext = Android.getApplication();
        String packageName;
        if (TextUtils.isEmpty(Android.getPackageName())) {
            packageName = mContext.getPackageName();
        } else {
            packageName = Android.getPackageName();
        }
        try {
            Class<?> loadClass = mContext.getClassLoader().loadClass(packageName + ".R");
            Class<?>[] classes = loadClass.getClasses();
            for (int i = 0; i < classes.length; i++) {
                if (classes[i].getName().equals(packageName + ".R$styleable")) {
    ...
    public static int[] getStyleableArryId(String name) {
        ...
        return getStyleableArryId(packageName, name);
    ...
    public static int getStyleableFieldId(String styleableName, String styleableFieldName) {
        ...
        return getStyleableFieldId(packageName, styleableName, styleableFieldName);
    }
