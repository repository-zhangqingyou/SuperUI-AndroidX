...
/**
 * Maintains an ordered put of {@link com.zqy.sutils.glide.load.model.ModelLoader}s and the model and data types they handle in
...
public class ModelLoaderRegistry {
  ...
  private <Model, Data> void tearDown(
       List<com.zqy.sutils.glide.load.model.ModelLoaderFactory<? extends Model, ? extends Data>> factories) {
  ...
  public synchronized <A> List<com.zqy.sutils.glide.load.model.ModelLoader<A, ?>> getModelLoaders( A model) {
    List<com.zqy.sutils.glide.load.model.ModelLoader<A, ?>> modelLoaders = getModelLoadersForClass(getClass(model));
    int size = modelLoaders.size();
    List<com.zqy.sutils.glide.load.model.ModelLoader<A, ?>> filteredLoaders = new ArrayList<>(size);
    //noinspection ForLoopReplaceableByForEach to improve perf
    for (int i = 0; i < size; i++) {
      com.zqy.sutils.glide.load.model.ModelLoader<A, ?> loader = modelLoaders.get(i);
  ...
  private <A> List<com.zqy.sutils.glide.load.model.ModelLoader<A, ?>> getModelLoadersForClass( Class<A> modelClass) {
    List<com.zqy.sutils.glide.load.model.ModelLoader<A, ?>> loaders = cache.get(modelClass);
    if (loaders == null) {
  ...
  private static class ModelLoaderCache {
    ...
    public <Model> void put(Class<Model> modelClass, List<com.zqy.sutils.glide.load.model.ModelLoader<Model, ?>> loaders) {
    ...
    @SuppressWarnings("unchecked")
    public <Model> List<com.zqy.sutils.glide.load.model.ModelLoader<Model, ?>> get(Class<Model> modelClass) {
      Entry<Model> entry = (Entry<Model>) cachedModelLoaders.get(modelClass);
      return entry == null ? null : entry.loaders;
    }

    private static class Entry<Model> {
      @Synthetic final List<com.zqy.sutils.glide.load.model.ModelLoader<Model, ?>> loaders;
