...
/**
 * A class that allows {@link com.zqy.sutils.glide.load.resource.transcode.ResourceTranscoder}s to be
...
public class TranscoderRegistry {
  ...
  /**
   * Registers the given {@link com.zqy.sutils.glide.load.resource.transcode.ResourceTranscoder} using
  ...
  public synchronized <Z, R> void register(
       Class<Z> decodedClass,  Class<R> transcodedClass,
       com.zqy.sutils.glide.load.resource.transcode.ResourceTranscoder<Z, R> transcoder) {
    transcoders.add(new Entry<>(decodedClass, transcodedClass, transcoder));
  }

  /**
   * Returns the currently registered
   * {@link com.zqy.sutils.glide.load.resource.transcode.ResourceTranscoder} for the given classes.
   *
  ...
  public synchronized <Z, R> com.zqy.sutils.glide.load.resource.transcode.ResourceTranscoder<Z, R> get(
       Class<Z> resourceClass,  Class<R> transcodedClass) {
    ...
    if (transcodedClass.isAssignableFrom(resourceClass)) {
      return (com.zqy.sutils.glide.load.resource.transcode.ResourceTranscoder<Z, R>) UnitTranscoder.get();
    }
    for (Entry<?, ?> entry : transcoders) {
      if (entry.handles(resourceClass, transcodedClass)) {
        return (com.zqy.sutils.glide.load.resource.transcode.ResourceTranscoder<Z, R>) entry.transcoder;
      }
    }
  ...
  public synchronized <Z, R> List<Class<R>> getTranscodeClasses(
       Class<Z> resourceClass,  Class<R> transcodeClass) {
  ...
  private static final class Entry<Z, R> {
    ...
    @Synthetic final com.zqy.sutils.glide.load.resource.transcode.ResourceTranscoder<Z, R> transcoder;
