...
public class SuperConstraintLayout extends ConstraintLayout implements DrawableImpl {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        int 右下圆角 = 0;
        if (attrs != null) {
            TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.SuperConstraintLayout);
            是否有按下效果 = typedArray.getBoolean(R.styleable.SuperConstraintLayout_zqy_click_effect, true);//默认有点击效果
            按下时透明度 = typedArray.getFloat(R.styleable.SuperConstraintLayout_zqy_click_alpha, 0.7f);//默认点击透明度
            填充颜色 = typedArray.getColor(R.styleable.SuperConstraintLayout_zqy_solid_color, Color.TRANSPARENT);
            边框颜色 = typedArray.getColor(R.styleable.SuperConstraintLayout_zqy_stroke_color, Color.TRANSPARENT);
            渐变开始颜色 = typedArray.getColor(R.styleable.SuperConstraintLayout_zqy_start_color, Color.TRANSPARENT);
            渐变结束颜色 = typedArray.getColor(R.styleable.SuperConstraintLayout_zqy_end_color, Color.TRANSPARENT);
            按下时填充颜色 = typedArray.getColor(R.styleable.SuperConstraintLayout_zqy_click_solid_color, Color.TRANSPARENT);
            按下时边框颜色 = typedArray.getColor(R.styleable.SuperConstraintLayout_zqy_click_stroke_color, Color.TRANSPARENT);
            字体颜色 = typedArray.getColor(R.styleable.SuperConstraintLayout_zqy_text_color, Color.GRAY);
            按下时字体颜色 = typedArray.getColor(R.styleable.SuperConstraintLayout_zqy_click_text_color, Color.TRANSPARENT);
            渐变模式 = typedArray.getInt(R.styleable.SuperConstraintLayout_zqy_gradient, Gradient.LINEAR_GRADIENT.ordinal());//默认 线性梯度(线性渐变)
            渐变方向 = typedArray.getInt(R.styleable.SuperConstraintLayout_zqy_orientation, GradientDrawable.Orientation.LEFT_RIGHT.ordinal());//默认从左到右
            边框宽度 = typedArray.getDimensionPixelSize(R.styleable.SuperConstraintLayout_zqy_stroke_width, 0);
            四圆角 = typedArray.getDimensionPixelSize(R.styleable.SuperConstraintLayout_zqy_radius, SizeUtils.dp2px(5));//默认圆角5dp
            左上圆角 = typedArray.getDimensionPixelSize(R.styleable.SuperConstraintLayout_zqy_top_left_radius, 0);
            右上圆角 = typedArray.getDimensionPixelSize(R.styleable.SuperConstraintLayout_zqy_top_right_radius, 0);
            左下圆角 = typedArray.getDimensionPixelSize(R.styleable.SuperConstraintLayout_zqy_bottom_left_radius, 0);
