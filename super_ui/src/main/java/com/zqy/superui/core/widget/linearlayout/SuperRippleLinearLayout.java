 ...
 * 描述:
 */
public class SuperRippleLinearLayout extends LinearLayout {

    private SuperRippleDrawable superRippleDrawable;

    public SuperRippleLinearLayout(Context context) {
        super(context);
        init(context, null);
    }

    public SuperRippleLinearLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(context, attrs);
    }

    public SuperRippleLinearLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }
    ...
    private void init(Context context, AttributeSet attrs) {
        if (attrs != null) {
            ...
            int 边框颜色 = typedArray.getColor(R.styleable.SuperRippleLinearLayout_zqy_stroke_color, Color.TRANSPARENT);
            int 渐变开始颜色 = typedArray.getColor(R.styleable.SuperRippleLinearLayout_zqy_start_color, Color.TRANSPARENT);
            int 渐变结束颜色 = typedArray.getColor(R.styleable.SuperRippleLinearLayout_zqy_end_color, Color.TRANSPARENT);
            int 渐变模式 = typedArray.getInt(R.styleable.SuperRippleLinearLayout_zqy_gradient, Gradient.LINEAR_GRADIENT.ordinal());//默认 线性梯度(线性渐变)
            int 渐变方向 = typedArray.getInt(R.styleable.SuperRippleLinearLayout_zqy_orientation, GradientDrawable.Orientation.LEFT_RIGHT.ordinal());//默认从左到右
            int 边框宽度 = typedArray.getDimensionPixelSize(R.styleable.SuperRippleLinearLayout_zqy_stroke_width, 0);
            int 四圆角 = typedArray.getDimensionPixelSize(R.styleable.SuperRippleLinearLayout_zqy_radius, (int) SuperUIManager.getApplication().getResources().getDimension(R.dimen._5sdp));//默认圆角5dp
            int 左上圆角 = typedArray.getDimensionPixelSize(R.styleable.SuperRippleLinearLayout_zqy_top_left_radius, 0);
            int 右上圆角 = typedArray.getDimensionPixelSize(R.styleable.SuperRippleLinearLayout_zqy_top_right_radius, 0);
            int 左下圆角 = typedArray.getDimensionPixelSize(R.styleable.SuperRippleLinearLayout_zqy_bottom_left_radius, 0);
            int 右下圆角 = typedArray.getDimensionPixelSize(R.styleable.SuperRippleLinearLayout_zqy_bottom_right_radius, 0);

            int 正常波纹颜色 = typedArray.getColor(R.styleable.SuperRippleLinearLayout_zqy_ripple_color, Color.TRANSPARENT);//正常波纹颜色
            int 长按颜色 = typedArray.getColor(R.styleable.SuperRippleLinearLayout_zqy_press_color, Color.TRANSPARENT);//长按颜色
