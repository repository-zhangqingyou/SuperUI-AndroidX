...
public class SuperRippleTextView extends AppCompatTextView {
    ...
    public SuperRippleTextView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(context, attrs);
    }

    public SuperRippleTextView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }
    ...
    private void init(Context context, AttributeSet attrs) {
        if (attrs != null) {
            TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.SuperRippleTextView);
            float 按下时透明度 = typedArray.getFloat(R.styleable.SuperRippleTextView_zqy_click_alpha, 0.7f);//默认点击透明度
            int 填充颜色 = typedArray.getColor(R.styleable.SuperRippleTextView_zqy_solid_color, Color.TRANSPARENT);
            int 边框颜色 = typedArray.getColor(R.styleable.SuperRippleTextView_zqy_stroke_color, Color.TRANSPARENT);
            int 渐变开始颜色 = typedArray.getColor(R.styleable.SuperRippleTextView_zqy_start_color, Color.TRANSPARENT);
            int 渐变结束颜色 = typedArray.getColor(R.styleable.SuperRippleTextView_zqy_end_color, Color.TRANSPARENT);
            int 字体颜色 = typedArray.getColor(R.styleable.SuperRippleTextView_zqy_text_color, Color.GRAY);
            int 按下时字体颜色 = typedArray.getColor(R.styleable.SuperRippleTextView_zqy_click_text_color, Color.TRANSPARENT);
            int 渐变模式 = typedArray.getInt(R.styleable.SuperRippleTextView_zqy_gradient, Gradient.LINEAR_GRADIENT.ordinal());//默认 线性梯度(线性渐变)
            int 渐变方向 = typedArray.getInt(R.styleable.SuperRippleTextView_zqy_orientation, GradientDrawable.Orientation.LEFT_RIGHT.ordinal());//默认从左到右
            int 边框宽度 = typedArray.getDimensionPixelSize(R.styleable.SuperRippleTextView_zqy_stroke_width, 0);
            int 四圆角 = typedArray.getDimensionPixelSize(R.styleable.SuperRippleTextView_zqy_radius, (int) SuperUIManager.getApplication().getResources().getDimension(R.dimen._5sdp));//默认圆角5dp
            int 左上圆角 = typedArray.getDimensionPixelSize(R.styleable.SuperRippleTextView_zqy_top_left_radius, 0);
            int 右上圆角 = typedArray.getDimensionPixelSize(R.styleable.SuperRippleTextView_zqy_top_right_radius, 0);
            int 左下圆角 = typedArray.getDimensionPixelSize(R.styleable.SuperRippleTextView_zqy_bottom_left_radius, 0);
